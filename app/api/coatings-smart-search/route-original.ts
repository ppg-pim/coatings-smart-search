...
// 🎯 NEW: Detect "which products are for X" type questions
function detectAttributeListQuestion(query: string): {
  isAttributeList: boolean;
  searchValue: string | null;
} {
  const lowerQuery = query.toLowerCase().trim()
  
  // Pattern 1: "which products are for [VALUE]"
  const whichForMatch = lowerQuery.match(/which products? (?:are|is) for ([a-z0-9\s]+)/i)
  if (whichForMatch) {
    return {
      isAttributeList: true,
      searchValue: whichForMatch[1].trim()
    }
  }
  
  // Pattern 2: "what products are [VALUE]"
  const whatAreMatch = lowerQuery.match(/what products? (?:are|is) ([a-z0-9\s]+)/i)
  if (whatAreMatch) {
    const value = whatAreMatch[1].trim()
    // Exclude generic words
    const excludeWords = ['available', 'there', 'in', 'the', 'database']
  ...
  // Pattern 3: "show me all [VALUE] products"
  const showAllMatch = lowerQuery.match(/show (?:me )?all ([a-z0-9\s]+) products?/i)
  if (showAllMatch) {
    return {
      isAttributeList: true,
      searchValue: showAllMatch[1].trim()
    }
  }
  
  // Pattern 4: "list all [VALUE] products"
  const listAllMatch = lowerQuery.match(/list all ([a-z0-9\s]+) products?/i)
  if (listAllMatch) {
    return {
      isAttributeList: true,
      searchValue: listAllMatch[1].trim()
    }
  }
...
// 🎯 Detect if user is asking about product models/families within a category
function detectModelListQuestion(query: string): {
  isModelList: boolean;
  targetAttribute: 'Product_Model' | 'family' | null;
  filterType: 'Product_Type' | 'family' | null;
  filterValue: string | null;
} {
  const lowerQuery = query.toLowerCase().trim()
  
  // Pattern 1: "how many product model(s) are/is associated to/with [TYPE]"
  const modelAssociatedMatch = lowerQuery.match(/how many product model(?:s)? (?:are |is )?associated (?:to|with) ([a-z0-9\s]+)/i)
  if (modelAssociatedMatch) {
    return {
      isModelList: true,
      targetAttribute: 'Product_Model',
      filterType: 'Product_Type',
      filterValue: modelAssociatedMatch[1].trim()
    }
  }
  
  // Pattern 2: "list all product models for/in/of [TYPE]"
  const modelForMatch = lowerQuery.match(/(?:list|show|what) (?:all )?product model(?:s)? (?:for|in|of|with) ([a-z0-9\s]+)/i)
  if (modelForMatch) {
    return {
      isModelList: true,
      targetAttribute: 'Product_Model',
      filterType: 'Product_Type',
      filterValue: modelForMatch[1].trim()
    }
  }
  
  // Pattern 3: "what product models are [TYPE]"
  const modelAreMatch = lowerQuery.match(/what product model(?:s)? (?:are|is) ([a-z0-9\s]+)/i)
  if (modelAreMatch) {
    return {
      isModelList: true,
      targetAttribute: 'Product_Model',
      filterType: 'Product_Type',
      filterValue: modelAreMatch[1].trim()
    }
  }
  
  // Pattern 4: "what families are/is [TYPE]"
  const familyMatch = lowerQuery.match(/what (?:product )?famil(?:y|ies) (?:are|is) ([a-z0-9\s]+)/i)
  if (familyMatch) {
    return {
      isModelList: true,
      targetAttribute: 'family',
      filterType: 'Product_Type',
      filterValue: familyMatch[1].trim()
    }
  }
  
  // Pattern 5: "how many families in [TYPE]"
  const familyInMatch = lowerQuery.match(/how many famil(?:y|ies) (?:in|for|of) ([a-z0-9\s]+)/i)
  if (familyInMatch) {
    return {
      isModelList: true,
      targetAttribute: 'family',
      filterType: 'Product_Type',
      filterValue: familyInMatch[1].trim()
    }
  }
...
async function handleMetaQuestion(
  ...
  if (filterInfo) {
    console.log(`🎯 With filter:`, filterInfo)
  }
  
  try {
    // 🎯 NEW: Handle attribute list questions (e.g., "which products are for military")
    if (type === 'list_by_attribute') {
      const { searchValue } = filterInfo
      
      console.log(`📊 Listing all products with attribute value: ${searchValue}`)
      
      // Search across Product_Type, Product_Model, and family
      const { data, error } = await supabaseClient
        .from('coatings')
        .select('family, Product_Type, Product_Model, sku')
        .or(
          `Product_Type.ilike.%${searchValue}%,` +
          `Product_Model.ilike.%${searchValue}%,` +
          `family.ilike.%${searchValue}%`
        )
        .limit(2000)
      ...
      // Group by family and collect metadata
      ...
      data?.forEach((row: any) => {
        ...
        if (family) {
          ...
          if (row.Product_Type) {
            entry.productTypes.add(row.Product_Type)
          }
          if (row.Product_Model) {
            entry.productModels.add(row.Product_Model)
          }
        }
      })
      
      const uniqueFamilies = Array.from(familyMap.keys()).sort()
      const totalProducts = data?.length || 0
      
      console.log(`✅ Found ${uniqueFamilies.length} families with "${searchValue}" (${totalProducts} total products)`)
      
      // Build summary
      let summary = `**Products with "${searchValue}"**\n\n`
      summary += `**Total:** ${uniqueFamilies.length} product families (${totalProducts} products)\n\n`
      
      if (uniqueFamilies.length > 0) {
        summary += `**Product Families:**\n`
        uniqueFamilies.slice(0, 100).forEach(family => {
          const familyData = familyMap.get(family)!
          const types = Array.from(familyData.productTypes)
          const models = Array.from(familyData.productModels)
          
          summary += `- **${family}** (${familyData.count} product${familyData.count > 1 ? 's' : ''})`
        ...
        if (uniqueFamilies.length > 100) {
          summary += `\n_...and ${uniqueFamilies.length - 100} more families_`
        }
      } else {
        summary += `⚠️ No products found matching "${searchValue}".`
      }
      
      return {
        success: true,
        questionType: 'meta',
        metaType: 'list_by_attribute',
        summary,
    ...
    // 🎯 Handle model/family list questions
    if (type === 'list_models') {
      const { targetAttribute, filterType, filterValue } = filterInfo
      
      console.log(`📊 Listing ${targetAttribute} for ${filterType} = ${filterValue}`)
      
      let query = supabaseClient
        .from('coatings')
        .select(`${targetAttribute}, family, sku, Product_Type`)
      
      // Apply filter
      if (filterType === 'Product_Type') {
        query = query.ilike('Product_Type', `%${filterValue}%`)
      } else if (filterType === 'family') {
        query = query.eq('family', filterValue)
      }
      ...
      // Group by target attribute and collect associated data
      const grouped = new Map<string, { families: Set<string>, count: number }>()
      
      data?.forEach((row: any) => {
        const attrValue = row[targetAttribute]
        if (attrValue) {
          if (!grouped.has(attrValue)) {
            grouped.set(attrValue, { families: new Set(), count: 0 })
          }
          const entry = grouped.get(attrValue)!
          entry.count++
          if (row.family) {
      ...
      const uniqueItems = Array.from(grouped.keys()).sort()
      const totalProducts = data?.length || 0
      
      console.log(`✅ Found ${uniqueItems.length} unique ${targetAttribute} with ${totalProducts} total products`)
      
      // Build summary
      const attributeName = targetAttribute === 'Product_Model' ? 'Product Models' : 'Product Families'
      let summary = `**${attributeName} for ${filterType} "${filterValue}"**\n\n`
      summary += `**Total:** ${uniqueItems.length} unique ${targetAttribute === 'Product_Model' ? 'models' : 'families'} (${totalProducts} products)\n\n`
      
      if (uniqueItems.length > 0) {
        summary += `**List:**\n`
        uniqueItems.slice(0, 100).forEach(item => {
          const itemData = grouped.get(item)!
          const families = Array.from(itemData.families)
          summary += `- **${item}**`
          if (targetAttribute === 'Product_Model' && families.length > 0) {
            summary += ` (${itemData.count} product${itemData.count > 1 ? 's' : ''}, Families: ${families.join(', ')})`
          } else {
            summary += ` (${itemData.count} product${itemData.count > 1 ? 's' : ''})`
          }
          summary += `\n`
        })
        
        if (uniqueItems.length > 100) {
          summary += `\n_...and ${uniqueItems.length - 100} more_`
        }
      } else {
        summary += `⚠️ No ${targetAttribute === 'Product_Model' ? 'models' : 'families'} found matching this ${filterType}.`
      }
      
      return {
        success: true,
        questionType: 'meta',
        metaType: 'list_models',
        summary,
        count: uniqueItems.length,
        totalProducts,
        items: uniqueItems,
        results: data || [],
        filter: { filterType, filterValue, targetAttribute }
      }
    }
...
async function fastMultiKeywordSearch(
  ...
  try {
    for (const keyword of keywords) {
      ...
      let familySearchTerm = keyword.replace(/\s+/g, '').toUpperCase()
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 🎯 PRIORITY 1: Check for attribute list questions (e.g., "which products are for military")
    const attributeListCheck = detectAttributeListQuestion(query)
    if (attributeListCheck.isAttributeList) {
      console.log(`> 🎯 Detected attribute list question: searching for "${attributeListCheck.searchValue}"`)
      ...
      const metaResult = await handleMetaQuestion(
        ...
        supabase,
        {
          searchValue: attributeListCheck.searchValue
        }
      )
    ...
    // 🎯 PRIORITY 2: Check for model/family list questions
    const modelListCheck = detectModelListQuestion(query)
    if (modelListCheck.isModelList) {
      console.log(`> 🎯 Detected model list question: ${modelListCheck.targetAttribute} for ${modelListCheck.filterType} = ${modelListCheck.filterValue}`)
      ...
      const metaResult = await handleMetaQuestion(
        ...
        supabase,
        {
          filterType: modelListCheck.filterType!,
          filterValue: modelListCheck.filterValue!,
          targetAttribute: modelListCheck.targetAttribute
        }
      )
    ...
    // PRIORITY 3: Check for regular meta-questions
    ...
    if (searchParams.questionType === "comparison") {
      ...
      searchKeywords.forEach(keyword => {
        ...
        const matchingProducts = cleanedProducts.filter(product => {
          const productFamily = normalizeFamily(product.family || '')
